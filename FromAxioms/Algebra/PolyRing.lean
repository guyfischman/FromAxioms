...
`Nf + Ng - 1 ≤ k`. Use that one when the index matters --- it is what
`polyMul_bound_sharp` and `isBoundOf_polyMul` are built on. This looser
statement is kept because its callers are satisfied by it and because
`Nf + Ng ≤ k` avoids the `Nat` subtraction. -/
...
/-! ### The rest of the least-index ladder, over an ideal
-/
...
`Finset.sum`, so ours cannot be applied to mathlib's carriers at all. The same
siting gap the comparator rung has been closing row by row.

THE FLOOR WAS GENUINELY ABSENT, MEASURED BEFORE IT WAS BUILT.
`grep -rn "^def sumUpto\|^def prodUpto\|^def sumRange\|^def foldUpto"
FromAxioms/` returned exactly one hit --- `Core.prodUpto`, a PRODUCT over `Nat`
only. There was no finite sum over an arbitrary carrier at any siting, so
`add_pow` could not even be STATED over a Lean type.

`NumberTheory.choose` IS CITED RATHER THAN COPIED, and the first draft got that
wrong. It landed a private `chooseT` duplicating `NumberTheory.choose`
(`Prime.lean`) on the belief that
`Prime.lean` was outside this file's cone. It is not: `Algebra.Field` imports it
and this file imports `Field`, so the name was reachable all along and the copy
was pure duplication. Checked in the other direction too --- neither `Prime` nor
`Arith` imports anything under `Algebra/`, so there is no cycle.

(`Nat.choose` is a MATHLIB name and Lean core has none, so the tree
defines its own; that part of the note stands.) -/
...
theorem listCoeff_mem_of_zero_mem {S zero : ZFSet.{u}} (hz : zero ∈ S) :
  ...
  | [], _, _ => hz
  | c :: _, hcs, 0 => hcs c List.mem_cons_self
  | c :: cs, hcs, k + 1 =>
    listCoeff_mem_of_zero_mem hz cs
      (fun x hx => hcs x (List.mem_cons_of_mem c hx)) k

theorem listCoeff_mem {R add mul zero one : ZFSet.{u}} (hR : IsRing R add mul zero one)
    (cs : List ZFSet.{u}) (hcs : ∀ c, c ∈ cs → c ∈ R) (k : Nat) :
    listCoeff zero cs k ∈ R :=
  listCoeff_mem_of_zero_mem hR.addGroup.mem_e cs hcs k
...
/-- Evaluating `X` returns the point: `X(x) = x`.

`evalAt_monomial` above is the whole proof: `polyX` is the degree-one monomial
with coefficient `one`. `polyComp_polyX` is the other direction, `f(X) = f`. -/
theorem evalAt_polyX {R add mul zero one x : ZFSet.{u}}
    (hR : IsRing R add mul zero one) (hx : x ∈ R) :
    evalAt R add mul zero one x (polyX R zero one) = x := by
  show evalAt R add mul zero one x (monomial R zero one 1) = x
  rw [evalAt_monomial hR hx hR.mem_one 1]
  show opAt mul one (opAt mul (gpow mul one x 0) x) = x
  rw [show gpow mul one x 0 = one from rfl, ringOne_mul hR hx,
    ringOne_mul hR hx]
...
/-- The `x`-side of the Weierstrass curve, `x^3 + a*x + b`, in `R[x]`. -/
def weierX (R add zero one a b : ZFSet.{u}) : ZFSet.{u} :=
  polyAdd R add (polyAdd R add (monomial R zero one 3) (monomial R zero a 1))
    (monomial R zero b 0)
...
def weierPoly (R add zero one a b : ZFSet.{u}) : ZFSet.{u} :=
  polyAdd (PolyRing R zero) (polyAddOp R add zero)
    (monomial (PolyRing R zero) (polyZero R zero) (polyOne R zero one) 2)
    (monomial (PolyRing R zero) (polyZero R zero)
      (polyNeg R add zero (weierX R add zero one a b)) 0)

#print axioms weierX
#print axioms weierPoly
...
/-- The `k`-th power of `X` is the monomial of degree `k`.

    gpow polyMulOp polyOne polyX k = monomial R zero one k

`monomial_mul_monomial` carries the whole step; `monomial_zero_eq_polyOne` is
the base. `polyX` is by definition the degree-one monomial with coefficient
`one`, so nothing here is more than bookkeeping --- but without it the
binomial expansion of `(X+1)^j` cannot be recognised as a sum of monomials,
and its coefficients cannot be read. -/
theorem gpow_polyX {R add mul zero one : ZFSet.{u}}
    (hR : IsRing R add mul zero one) :
    ∀ k : Nat, gpow (polyMulOp R add mul zero) (polyOne R zero one)
        (polyX R zero one) k
      = monomial R zero one k
  | 0 => monomial_zero_eq_polyOne.symm
  | k + 1 => by
    show opAt (polyMulOp R add mul zero)
      (gpow (polyMulOp R add mul zero) (polyOne R zero one)
        (polyX R zero one) k) (polyX R zero one) = _
    rw [gpow_polyX hR k, polyX,
      opAt_polyMulOp hR ((mem_polyRing_iff _ _ _).mpr
          (isPolyOver_monomial hR hR.mem_one k))
        ((mem_polyRing_iff _ _ _).mpr (isPolyOver_monomial hR hR.mem_one 1)),
      monomial_mul_monomial hR hR.mem_one hR.mem_one k 1,
      hR.mul_one one hR.mem_one]

#print axioms gpow_polyX
...
which is what lets one vector serve both, and is checked rather than assumed:
had they overlapped, the two lemmas could never be used together and neither
statement would say so.
-/
...
THE FLOOR IS LOWER THAN MATHLIB'S SIGNATURE SUGGESTS, and reading OUR side is
what showed it. From mathlib's `Fin`-indexed matrices and `Equiv.Perm` signs I
priced this as rebuilding matrices and permutations over Lean types --- a layer.
But `matMinor` here is
...
/-- A scalar matrix factoring through a singular one forces the scalar
nilpotent.

If `E * C = c * I` and `det E = zero`, then `c^n = zero`: take determinants, and
...
theorem ringPow_eq_zero_of_matMulOn_scalar {R add mul zero one c : ZFSet.{u}}
    (hR : IsRing R add mul zero one) (hc : c ∈ R)
    {E C : Nat → Nat → ZFSet.{u}} (hE : ∀ i m, E i m ∈ R) (hC : ∀ i m, C i m ∈ R)
    {n : Nat} (hn : 0 < n)
    (hdet : detN R add mul zero one E n = zero)
    (heq : ∀ i j, i < n → j < n →
      matMulOn add mul zero E C n i j = opAt mul c (idMat zero one i j)) :
    ringPow mul one c n = zero := by
  have hstep : ringPow mul one c n
      = detN R add mul zero one (matMulOn add mul zero E C n) n :=
    (detN_scalar hR hc n).symm.trans
      (detN_congr_lt n (fun i j hi hj => (heq i j hi hj).symm))
  rw [hstep, detN_mul hR hE hC hn, hdet,
    ringZero_mul hR (detN_mem hR hC n)]
...
theorem matMulOn_neg_right {R add mul zero one : ZFSet.{u}}
    ...
    matMulOn add mul zero A (fun a b => ringNeg R add zero (C a b)) n m j
      = ringNeg R add zero (matMulOn add mul zero A C n m j) :=
  (foldF_congr n (fun a _ => ringMul_neg hR (hA m a) (hC a j))).trans
    (foldF_neg hR (fun a => mulAt_mem hR (hA m a) (hC a j)) n).symm
...
theorem matPow_add {R add mul zero one : ZFSet.{u}}
    ...
    ∀ (t i l : Nat), i < n → l < n →
      matPow add mul zero one n A (k + t) i l
        = matMulOn add mul zero (matPow add mul zero one n A k)
            (matPow add mul zero one n A t) n i l
  | 0, i, l, _, hl => (matMulOn_idMat hR (matPow_mem hR hA n k) i hl).symm
  | t + 1, i, l, hi, hl => by
    have hK : ∀ a m, matPow add mul zero one n A k a m ∈ R := matPow_mem hR hA n k
    have hT : ∀ a m, matPow add mul zero one n A t a m ∈ R := matPow_mem hR hA n t
    show matMulOn add mul zero (matPow add mul zero one n A (k + t)) A n i l
      = matMulOn add mul zero (matPow add mul zero one n A k)
...
theorem exists_dirichlet_collision_lt {A : Nat → Nat} {b Q r : Nat}
    ...
    ∃ k k' : Nat, k < k' ∧ k' < Q ^ r + 1 ∧
      ∀ i, i < r →
        (k * A i % b) * Q / b = (k' * A i % b) * Q / b := by
  have hbox : ∀ k i : Nat, (k * A i % b) * Q / b < Q := fun k i =>
    Core.div_lt_of_lt_mul'
      (Nat.mul_lt_mul_of_lt_of_le (Nat.mod_lt _ hb) (Nat.le_refl Q) hQ)
  obtain ⟨k, k', hlt, hk', heq⟩ :=
    exists_repeat_of_finite_lt
      (G := powSet (ofNat.{0} Q) r)
      (F := fun k => tupleOf (fun i => ofNat.{0} ((k * A i % b) * Q / b)) r)
      (fun k => tupleOf_mem r (fun i _ =>
        (mem_ofNat_iff _ Q).mpr ⟨_, hbox k i, rfl⟩))
      (equinumerous_powSet (equinumerous_refl (ofNat.{0} Q)) r)
  refine ⟨k, k', hlt, hk', fun i hi => ?_⟩
  have hco := congrArg (fun t => tupleCoeff t r i) heq
  simp only [tupleCoeff_tupleOf r i hi] at hco
  exact ofNat_injective hco
...
statement and this form asserts only that two multipliers somewhere agree, which
is true of any function into a finite set. -/
theorem exists_dirichlet_collision {A : Nat → Nat} {b Q r : Nat}
    (hb : 0 < b) (hQ : 0 < Q) :
    ∃ k k' : Nat, k < k' ∧
      ∀ i, i < r →
        (k * A i % b) * Q / b = (k' * A i % b) * Q / b :=
  let ⟨k, k', hlt, _, heq⟩ := exists_dirichlet_collision_lt (A := A) hb hQ (r := r)
  ⟨k, k', hlt, heq⟩
